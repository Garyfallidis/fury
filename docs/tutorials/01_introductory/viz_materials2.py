...
def main():
    ...
    renderer.AddActor(actor.point(np.array([[0, 1, 0.2]]), (1, 0, 0)))

    renderer.AddActor(actor.point(np.array([[1, 1, 1.]]), (1, 1, 0)))
    ...
    # from fury import actor as act

    # actor = act.cube(centers=np.array([[0, 0, 0.]]), directions=(1, 0, 0), colors=(1, 0, 0), scales=(1, 2, 3))
    ...
    plane.SetXLength(10 * (bounds[1] - bounds[0] + (rnge[0] * expand)))
    plane.SetYLength(THICKNESS)
    plane.SetZLength(10 * (bounds[5] - bounds[4] + (rnge[2] * expand)))
